...
percentile_summary <- R6Class(
  ...
  public = list(
    ...
    #'
    initialize = function(perc = 0, perc_flag = NULL) {
    ...
    get_inp_lines = function(delimiter = " ") {
      ...
      checkmate::assert_numeric(self$report_percentile, lower = 0, upper = 100)
  ...
  active = list(
    ...
    json_list_object = function() {
      return(list(
        perc_report_value = self$report_percentile
      ))
    },
  ...
  private = list(
    ...
    validate_perc_flag = function(perc_flag_param) {
      ...
      if (isFALSE(is.null(perc_flag_param$op$enable_percentile_summary))) {
        warning(paste0(
          "Initializing percentile summary with a non-null ",
          private$name_options_flag,
          " value"
        ))
      }
